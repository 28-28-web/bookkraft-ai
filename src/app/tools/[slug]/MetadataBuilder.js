...
export default function MetadataBuilder() {
    ...
    const checks = useMemo(() => {
        ...
        results.push({
            name: 'Title',
            status: form.title ? 'pass' : 'fail',
            detail: form.title ? 'Title is present.' : 'Missing title — KDP will reject your upload without a book title.',
            fixHint: 'Add your book title in the Title field.',
        });

        results.push({
            name: 'Author',
            status: form.authors ? 'pass' : 'fail',
            detail: form.authors ? 'Author name present.' : 'Missing author — KDP requires at least one author name.',
            fixHint: 'Add your name in the Author field.',
        });

        results.push({
            name: 'BISAC Category',
            status: form.bisacCategory1 ? 'pass' : 'fail',
            detail: form.bisacCategory1 ? `Category set: ${form.bisacCategory1}` : 'No BISAC category — without this, KDP places your book incorrectly and readers can\'t find it.',
            fixHint: 'Set at least one BISAC category.',
        });
        ...
        results.push({
            ...
            detail: keywords.length >= 3
                ? `${keywords.length} keywords set. Good for discoverability.`
                : keywords.length > 0
                    ? `Only ${keywords.length} keyword(s). KDP allows 7 — use all of them for better search visibility.`
                    : 'No keywords — you\'re leaving discoverability on the table. KDP gives you 7 keyword slots.',
            fixHint: 'Add at least 5–7 relevant keywords.',
        });

        results.push({
            name: 'Short Description',
            status: form.shortDesc && form.shortDesc.length >= 50 ? 'pass' : form.shortDesc ? 'warn' : 'fail',
            detail: form.shortDesc && form.shortDesc.length >= 50
                ? 'Short description looks good.'
                : form.shortDesc
                    ? 'Short description is too brief — aim for at least 50 characters to pass KDP review.'
                    : 'Missing short description — this shows on your book\'s product page.',
            fixHint: 'Write a compelling 1–2 sentence description.',
        });

        results.push({
            name: 'Long Description',
            status: form.longDesc && form.longDesc.length >= 200 ? 'pass' : form.longDesc ? 'warn' : 'fail',
            detail: form.longDesc && form.longDesc.length >= 200
                ? `Long description: ${form.longDesc.length} characters. Good length.`
                : form.longDesc
                    ? `Long description is only ${form.longDesc.length} chars. Aim for 200–4000 characters for best results.`
                    : 'No long description — this is your main sales copy. Missing it hurts conversions.',
            fixHint: 'Write a full book description (200–4000 characters).',
        });

        results.push({
            name: 'ISBN / Identifier',
            status: form.isbn || form.asin ? 'pass' : 'warn',
            detail: form.isbn || form.asin
                ? `Identifier present: ${form.isbn || form.asin}`
                : 'No ISBN or ASIN — not required for KDP, but needed for IngramSpark and wide distribution.',
            fixHint: 'Add your ISBN if publishing wide.',
        });
  ...
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    ...
    return (
            ...
            <div className="tool-layout">
                <div className="tool-input-card" style={{ maxHeight: '600px', overflowY: 'auto' }}>
                    ...
                    <button
                        ...
                        style={{
                            width: '100%', marginTop: '16px', background: '#1a1a1a', color: '#fff',
                            padding: '13px', borderRadius: '8px', fontWeight: 600, fontSize: '1rem',
                            border: 'none', cursor: form.title || form.authors ? 'pointer' : 'not-allowed',
                            opacity: form.title || form.authors ? 1 : 0.5,
                        }}
                    >
                        Check My Metadata →
                    </button>
                ...
                <div className="tool-output-card">
                    ...
                    <p style={{ color: '#6b7280', fontSize: '0.9rem', marginBottom: '16px' }}>
                        Fill in the form and click "Check My Metadata" to get your full report and formatted output.
                    </p>
                        ...
                        <div style={{ background: '#1a1a1a', color: '#fff', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {checks.filter(c => c.status === 'fail' || c.status === 'warn').map((item, i) => (
                                    <div
                                        key={i}
                                        style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', background: 'rgba(255,255,255,0.07)', borderRadius: '7px', padding: '10px 14px', fontSize: '0.88rem' }}
                                    >
                            ...
                            <div key={i} className={`val-check ${statusClass(c.status)}`}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    {(c.status === 'fail' || c.status === 'warn') && (
                                        <p style={{ margin: '4px 0 0 0', fontSize: '0.83rem', color: '#b8860b', fontWeight: 600 }}>
                                            → {c.fixHint}
                                        </p>
                                    )}
                                </div>
                    ...
                    {hasCoreFields && (
                        <div style={{ background: '#faf9f7', border: '2px solid #C9933A', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
                            <h4 style={{ fontSize: '1rem', fontWeight: 700, color: '#1a1a1a', marginBottom: '8px' }}>
                                ✅ Metadata done. Next step:
                            </h4>
                            <p style={{ fontSize: '0.9rem', color: '#666', marginBottom: '16px' }}>
                                Build your table of contents. KDP requires a TOC for every ebook.
                            </p>
                            <a href="/tools/toc-generator" style={{ display: 'inline-block', background: '#C9933A', color: '#fff', padding: '10px 20px', borderRadius: '8px', textDecoration: 'none', fontWeight: 600, marginRight: '12px' }}>
                                Generate Table of Contents →
                            </a>
                            <a href="/signup?plan=pro" style={{ display: 'inline-block', color: '#b8860b', fontWeight: 600, textDecoration: 'none', fontSize: '0.9rem' }}>
                                or get all 12 tools →
                            </a>
                        </div>
                    )}
        ...
        <div className="seo-content" style={{ maxWidth: '800px', margin: '3rem auto', padding: '0 1rem' }}>
            ...
            <p>The Metadata Builder guides you through every metadata field step by step — title, subtitle, description, categories, BISAC codes, keywords, and more. Fill in the form, get an instant quality check, then export your metadata formatted for each platform.</p>

            <h2>Works Across Multiple Platforms</h2>
            <p>Publishing on more than one platform means managing slightly different metadata requirements for each one. The Metadata Builder accounts for platform-specific differences. Supported platforms include Amazon KDP, IngramSpark, Draft2Digital, Smashwords, and Barnes and Noble Press.</p>
...
// Updated: email gate + validation checks + guided fix chain + proactive CTAs