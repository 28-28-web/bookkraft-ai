export const EPUB_ERRORS = [
  ...
  },
  {
    ...
    cause: '<p>RSC-005 on the opf:role attribute fires when an EPUB 2 metadata construct appears in a file declaring EPUB 3 — EPUB 3 replaced opf:role with a different structure, so EPUBCheck 5.x treats it as an invalid attribute. Files compiled from Scrivener using its EPUB 2 project template, or exported from older Calibre versions, are the most common source of this error.</p>',
  ...
  },
  {
    ...
    cause: '<p>Kobo flags this error when the <code>&lt;dc:title&gt;</code> element in content.opf is empty — either left blank during conversion or never populated from a template before export. The fix is opening content.opf and adding your book title between the <code>&lt;dc:title&gt;&lt;/dc:title&gt;</code> tags, then re-exporting or re-validating.</p>',
  ...
  },
  {
    ...
    cause: '<p>A literal <code>&amp;</code> character in EPUB chapter files causes a fatal XML parse error — the file cannot be processed at all, not just flagged as a warning. Every <code>&amp;</code> must be written as <code>&amp;amp;</code> in EPUB content because EPUB is XHTML, not HTML, and XHTML follows strict XML rules. The most common sources are URLs with query strings, titles containing ampersands pasted from Word, and OPF metadata fields like publisher names.</p>',
  ...
  },
  {
    ...
    cause: '<p>OPF-048 fires when the <code>unique-identifier</code> attribute on the OPF <code>&lt;package&gt;</code> element names a cross-reference ID that doesn\'t match any <code>&lt;dc:identifier&gt;</code> element in the metadata block. This is not a missing ISBN — the identifier may be present and correctly formatted, but the attribute names used as the cross-reference don\'t agree. It usually happens when a template is edited on one side without updating the other.</p>',
